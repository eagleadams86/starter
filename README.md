...
| **A focus never hides under the header** | The header stays pinned while you scroll, and the browser's own scroll to a focused control — **Tab**, and above all **Shift+Tab** back up the page — would line that control up with the top of the window, right behind the bar. `html { scroll-padding-top }` from the header's **measured** height (a ResizeObserver writes `--head-h`, since the bar changes depth with the window, zoom and the name wrapping) moves the top of the window below it, with 8px to spare for the focus ring. On a phone, where the header scrolls away instead, it is 0. If your app pins more chrome under the header, measure that into the same number; don't also add `scroll-margin-top` to controls, or the two add together |
